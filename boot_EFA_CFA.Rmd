...
cat("Total number of rows at the beginning:", initial_row_count, "\n")
...
cat("Number of rows with all item values missing:", all_missing_rows, "\n")

# Remove rows where all questionnaire items are NA
data <- data %>%
  filter(rowSums(is.na(.[, questionnaire_vars])) < length(questionnaire_vars))

# Count the number of remaining rows after removing those with all questionnaire items missing
remaining_rows_after_all_na_removal <- nrow(data)
cat("Number of rows after removing those with all questionnaire items missing:", remaining_rows_after_all_na_removal, "\n")
...
# Count rows with at least one NA item
some_missing_rows <- sum(rowSums(is.na(data[, questionnaire_vars])) > 0)
cat("Number of rows with one or more item values missing:", some_missing_rows, "\n")

# Perform k-NN imputation on the subset of data
imputed_data <- kNN(data[, questionnaire_vars]) # Default k=5, but adjust as needed

# For each variable, replace only the values that were imputed
for (var in questionnaire_vars) {
  imp_col_name <- paste0(var, "_imp")
  # Identify the indices of the values that were imputed
  imputed_indices <- which(imputed_data[[imp_col_name]] == TRUE)
  # Replace only the values that were imputed in the main dataset
  data[imputed_indices, var] <- imputed_data[imputed_indices, var]
}

# Descriptive statistics after imputation
cleaned_missing_values <- calc_missing_values(data, questionnaire_vars)
cat("Total missing values after cleaning:", cleaned_missing_values, "\n")
...
cfa_model <- cfa(cfa_model_syntax, data = cfa_data_rmv, missing = "fiml")
summary(cfa_model, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)

# Bootstrapping the CFA model
boot_cfa_model <- bootstrapLavaan(cfa_model, R = boot_iterations, iseed = iseed, FUN = "fitMeasures", fit.measures = c("chisq", "cfi", "tli", "rmsea", "srmr"))
...
# Construct the CFA full model syntax by having all items load on a single common latent factor
latent_factor <- "common_factor"
cfa_full_model_syntax <- paste(latent_factor, "=~", paste(questionnaire_vars, collapse = " + "))

# Print the CFA full model syntax for verification
print(cfa_full_model_syntax)

# Fit the unrestricted (full) model
full_cfa_model <- cfa(cfa_full_model_syntax, data = cfa_data, missing = "fiml")

# Compute bootstrap LRT p-value to compare the models
# pvalue_LRT <- bootstrapLRT(h0 = cfa_model, h1 = full_cfa_model, R = boot_iterations)
pvalue_LRT <- bootstrapLRT(h0 = full_cfa_model, h1 = cfa_model, R = boot_iterations, type = "nonparametric")
