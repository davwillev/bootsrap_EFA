...
```{r}
# Set file path
path <- "/Users/davidevans/Library/CloudStorage/OneDrive-Personal/My Projects/UBham/Disability attribution/Analyses/Factor analysis/7-item_ADLInterferenceAndAt-UniversalDisabilityI_DATA_2023-08-31_1142.csv"
...
# Create an empty vector to store custom labels
custom_labels <- vector("character", length(initial_cfa_params))

# Generate the custom labels using a basic loop
for (i in 1:length(initial_cfa_params)) {
  param <- initial_cfa_params[i]
  if (param %in% names(boot_param_means)) {
    custom_labels[i] <- as.character(round(boot_param_means[param], 2))
  } else {
    custom_labels[i] <- param
  }
}

# Print the custom_labels to see if they were generated correctly
print(custom_labels)

# Create the mermaid.js format for the CFA model

# Create the mermaid diagram string
mermaid_diagram <- "graph TD;"

# Specify node shapes for latent variables
for (node in latent_nodes) {
  mermaid_diagram <- paste0(mermaid_diagram, node, "[(", node, ")]\n")
}

# Specify node shapes for observed variables
for (node in observed_nodes) {
  mermaid_diagram <- paste0(mermaid_diagram, node, "{", node, "}\n")
}

# Add factor loadings with custom labels
for (i in 1:length(factor_rels)) {
  mermaid_diagram <- paste0(mermaid_diagram, factor_rels[i], "|", custom_labels[i], "|;\n")
}

# Display the diagram with DiagrammeR
DiagrammeR::DiagrammeR(diagram = mermaid_diagram)
