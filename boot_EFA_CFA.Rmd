...
```{r}
# Set file path
path <- "/Users/davidevans/Library/CloudStorage/OneDrive-Personal/My Projects/UBham/Disability attribution/Analyses/Factor analysis/ADLInterferenceAndAt-UniversalDisabilityI_DATA_2023-08-31_1142.csv"
...
cat("Total number of rows at beginning:", initial_row_count, "\n")

# Initial count of missing values
total_missing_values <- calc_missing_values(data, questionnaire_vars)
cat("Total missing values before cleaning:", total_missing_values, "\n")
...
cat("Number of rows with all values missing:", all_missing_rows, "\n")
...
cat("Number of rows after removing those with all values missing:", remaining_rows_after_all_na_removal, "\n")
...
writeLines(alpha_output_with_title, "Chronbachs_Alpha_Whole_Dataset.txt")
...
writeLines(alpha_output_with_title, "Chronbachs_Alpha_other.txt")
...
ggsave("boot_scree.pdf", boot_scree, width = 11.69, height = 8.27) # Save to PDF (A4)
...
ggsave("nfactors_hist.pdf", grid, width = 11.69, height = 8.27) # Save to PDF (A4)
...
if(is.null(initial_efa)){
  ...
  writeLines(efa_output_with_title, "Initial_EFA.txt")
...
```{r}
  ...
  write.csv(loadings_summary_unrotated, "Loadings_before_Procrustes.csv", row.names = FALSE)
  ...
  # Save summary to a CSV file
  write.csv(formatted_loadings_summary_rotated, "Loadings_after_Procrustes.csv", row.names = FALSE)
  ...
  # Save summary to a CSV file
  write.csv(variance_results, "Explained_variance.csv", row.names = FALSE)
  ...
  # Save to a CSV fikle
  write.csv(communalities_summary_rotated, "Communalities.csv", row.names = FALSE)
...
cfa_model <- lavaan::cfa(cfa_model_syntax, data = cfa_data_rmv, missing = "fiml")
initial_cfa <- summary(cfa_model, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)

# Capture printed output of the initial CFA model
cfa_output <- capture.output(print(initial_cfa))

# Bootstrapping the initial CFA model
boot_cfa_model <- lavaan::bootstrapLavaan(cfa_model, R = boot_iterations, iseed = seed, FUN = "fitMeasures", fit.measures = c("chisq", "cfi", "tli", "rmsea", "srmr", "aic", "bic"))

# Get summary of the bootstrapped CFA model
boot_cfa <- summary(boot_cfa_model, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)

# Capture printed output of the bootstrapped CFA
boot_cfa_output <- capture.output(print(boot_cfa))

# Extract the bootstrapped distributions for each measure
boot_measures <- list(
  chisq = boot_cfa_model[, "chisq"],
  cfi = boot_cfa_model[, "cfi"],
  tli = boot_cfa_model[, "tli"],
  rmsea = boot_cfa_model[, "rmsea"],
  srmr = boot_cfa_model[, "srmr"],
  aic = boot_cfa_model[, "aic"],
  bic = boot_cfa_model[, "bic"]
)

# Calculate the 2.5th and 97.5th percentiles for the 95% confidence interval for each measure
ci_measures <- lapply(boot_measures, function(measure) {
  compute_ci(measure)
})

# Get the test statistic and degrees of freedom for the original sample
T_orig <- fitMeasures(cfa_model, "chisq")
df_orig <- fitMeasures(cfa_model, "df")

# Compute a bootstrap based p-value for chi-square
pvalue_boot <- length(which(boot_measures$chisq > T_orig)) / length(boot_measures$chisq)
...
writeLines(final_output, "CFA_Summary.txt")

knitr::opts_chunk$set(echo = TRUE)
```

### Compare full and restricted models

```{r}

# Construct the CFA full model syntax by having all items load on a single common latent factor (MR1)
latent_factor <- "MR1"
cfa_full_model_syntax <- paste(latent_factor, "=~", paste(questionnaire_vars, collapse = " + "), "\n")

# Print the CFA full (unrestricted, h1) model syntax for verification
print(cfa_full_model_syntax)

# Fit the full (unrestricted, h1) model
full_cfa_model <- lavaan::cfa(cfa_full_model_syntax, data = cfa_data, missing = "fiml")

# Compute bootstrap LRT p-value to compare the restricted and unrestricted models
pvalue_LRT <- lavaan::bootstrapLRT(h0 = cfa_model, h1 = full_cfa_model, R = boot_iterations, type = "nonparametric")

# Print the bootstrap LRT p-value
print(paste0("Bootstrap LRT chi-square p-value: ", pvalue_LRT))
