...
```{r}
# Set the file path
path <- "/Users/davidevans/OneDrive/My Projects/UBham/Disability attribution/Analyses/Factor analysis/ADLInterferenceAndAt-UniversalDisabilityI_DATA_2023-07-14_0928.csv"
...
boot_cfa_model <- bootstrapLavaan(cfa_model, R = boot_iterations, FUN = "fitMeasures", fit.measures = "chisq")
...
boot_cfa_ci_lower <- quantile(boot_cfa_model, 0.025)
boot_cfa_ci_upper <- quantile(boot_cfa_model, 0.975)

print(paste0("95% Confidence interval for the chi-square statistic: [", boot_cfa_ci_lower, ", ", boot_cfa_ci_upper, "]"))

# Get the test statistic for the original sample
T_orig <- fitMeasures(cfa_model, "chisq")

# Compute a bootstrap based p-value
pvalue_boot <- length(which(boot_cfa_model > T_orig))/length(boot_cfa_model)
print(paste0("Bootstrap p-value: ", pvalue_boot))
